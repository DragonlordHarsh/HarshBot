...
function eventsOnDate(targetDate) {
  return DATA.events.filter(e => e.date === targetDate)
...
document.getElementById('composer').addEventListener('submit', (e) => {
  ...
  } else if (/day after tomorrow/i.test(q) || /after 2 days/i.test(q)) {
  ...
  if (/faculty|professor|teacher/i.test(q)) {
    if (DATA.sections.some(s => q.toUpperCase().includes(s))) {
